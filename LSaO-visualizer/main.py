...
def live_spectrum_dB(block, channel, res_width, res_height, xlow, xhigh, min_dB, style, thickness):
    ...
    fsong_res = 20*np.log10(np.maximum(fsong_res, 1e-12))
...
def live_spec_balance(block, res_width, res_height, xlow, xhigh, style, thickness):
    ...
    fsongL_res_2 = np.log10(np.maximum(fsongL_res, 1e-12))
    fsongR_res_2 = np.log10(np.maximum(fsongR_res, 1e-12))
    fsongL_res_2 = np.log(np.e**10 + np.e**fsongL_res_2) #soft clipping
    fsongR_res_2 = np.log(np.e**10 + np.e**fsongR_res_2) #soft clipping
...
def live_envelope(block, channel, res_width, res_height,smoothing, style, thickness):
    ...
    # ~3s of history at 60 fps, stretched across the full width (not 800 frames).
    hist = max(32, min(int(res_width), 180))
    if oldEnvLive.shape[0] != hist:
        oldEnvLive = np.zeros(hist, dtype=np.float64)

    oldEnvLive = np.roll(oldEnvLive, -1)
    peak = float(np.max(np.abs(audio))) if audio.size else 0.0
    sm = max(float(smoothing), 1.0)
    oldEnvLive[-1] = peak / (1 + sm / 1000.0) + oldEnvLive[-2] * (1 - 1 / (1 + sm / 1000.0))

    fsong_comp = np.clip(oldEnvLive * 0.95 * res_height, 0, res_height - 1)
...
def generate_polar(output_name,input_audio,channel,fps, res_width, res_height,offset, note, interpolation,thickness,compression, callback_function):
    ...
    audioLInterp = np.nan_to_num(audioLInterp, nan=0.0, posinf=0.0, neginf=0.0)
    audioRInterp = np.nan_to_num(audioRInterp, nan=0.0, posinf=0.0, neginf=0.0)
...
def _map_samples_to_axis(samples, size, amplitude=31130.0):
    """Map audio in [-1, 1] onto a screen axis without float16 overflow."""
    x = np.nan_to_num(np.asarray(samples, dtype=np.float32), nan=0.0, posinf=0.0, neginf=0.0)
    mapped = (x * amplitude + 32768.0) * (int(size) - 1) / 65535.0
...
def live_polar(block,channel,res_width, res_height, offset, note,interpolation,thickness):
    ...
    audioLInterp = _map_samples_to_axis(audioLInterp, res_height)
    audioRInterp = _map_samples_to_axis(audioRInterp, res_width)
    for m in range(len(audioLInterp)):
        frameData[int(audioLInterp[m]),int(audioRInterp[m])] = True
    frameData = apply_thickness(frameData, thickness)
    return frameData
...
def live_recurrence(block,channel, res_width, res_height, threshold, thickness):
    ...
    nW, nH = max(int(res_width), 2), max(int(res_height), 2)
    audioL = np.clip(signal.resample(np.asarray(audioL, dtype=np.float64), nH), -1, 1)
    audioR = np.clip(signal.resample(np.asarray(audioR, dtype=np.float64), nW), -1, 1)
    distances = np.abs(audioL[np.newaxis, :] - audioR[:, np.newaxis])
    if threshold >= 0:
        frameData = distances < threshold
    else:
        frameData = ~(distances < -threshold)
...
def live_chladni(block,channel, res_width, res_height, mode, zoom, exp_filter, threshold, thickness):
    ...
    envL_past = envL
    envR_past = envR
    envL = np.maximum(envL, 1e-3)
    envR = np.maximum(envR, 1e-3)
...
def live_poincare(block,channel,res_width, res_height,delay,interpolation,thickness):
    ...
    audioLInterp = _map_samples_to_axis(audioLInterp, res_height, amplitude=32768.0)
    audioRInterp = _map_samples_to_axis(audioRInterp, res_width, amplitude=32768.0)
    for m in range(len(audioLInterp)):
        frameData[int(audioLInterp[m]),int(audioRInterp[m])] = True
    frameData = apply_thickness(frameData, thickness)
    return frameData
